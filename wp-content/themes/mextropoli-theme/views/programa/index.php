...
<section class="programa" id="app">
    ...
    <div class="descarga">
        <a href="<?php the_field('pdf_descarga'); ?>" target="_blank">Descarga el programa de mano</a>
    </div>
    ...
    <div class="loop-programa">
        <div class="grid__container">
            <div class="loop-programa__contenedor">
                ...
                <div class="card__full" v-for="day in allDays">
                    <div class="card-day__fecha toggable" style="text-transform: uppercase;">
    ...
    new Vue({
        ...
        mounted() {
            ...
            if(window.location.hash) {
                ...
                var event =  this.store.events.find(function(ev){
                    return ev.slug == initialSlug;
                });
        ...
        computed: {
            ...
            filtered() {
                ...
                if(this.selectedCategoryId > 0){
                    var app =this;
                    events = events.filter(function(event){ return event.categories.includes(app.selectedCategoryId)})
                }

                if(this.selectedSpeakerId > 0){
                    var app =this;
                    events = events.filter(function(event){return event.event_speaker.includes(app.selectedSpeakerId)})
                }

                if(this.selectedTicketId > 0){
                    var app =this;
                    events = events.filter(function(event){return event.ticket_id == app.selectedTicketId})
                }

                if(this.selectedVenueId > 0){
                    var app =this;
                    events = events.filter(function(event){return event.venue_id == app.selectedVenueId})
                }
            ...
            selectedCategory() {
                var app = this;
                return this.store.categories.find(function(category){return category.term_id === app.selectedCategoryId})
            },
            selectedSpeaker() {
                var app = this;
                return this.store.speakers.find(function(speaker){return speaker.ID === app.selectedSpeakerId})
            },
            selectedVenue() {
                var app = this;
                return this.store.venues.find(function(venues){return venues.ID === app.selectedVenueId})
            },
            selectedTicket() {
                var app = this;
                return this.store.tickets.find(function(ticket){return ticket.ID === app.selectedTicketId})
            },
        },
        methods: {
            filterByDay(day){
                return this.filtered.filter(function(event){ return event.date === day})
            },
            openModal(event){
                ...
                this.modal.speakers = this.store.speakers.filter(function(speaker){ return  event.event_speaker.includes(speaker.ID)})

                // Find the venue of this event.
                this.modal.venue = this.store.venues.find(function(venue){ return  event.venue_id == venue.ID})

                // Find the ticket of this event.
                this.modal.ticket = this.store.tickets.find(function(ticket){ return  event.ticket_id == ticket.ID})
            ...
            moment,
            orderByTime(events) {
                return R.sortBy(function(event){ return event.time})(events)
            },
