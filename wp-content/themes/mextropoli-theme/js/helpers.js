function removeParam(sParam)
{
    ...
    var sURLVariables = decodeURIComponent(window.location.search.substring(1)).split('&').filter(function(item){item != ""}),
        sParameterName,
        i;
...
function insertParam(sParam, value)
{
    ...
    var url = window.location.href.split('?')[0]+'?';
    var sURLVariables = decodeURIComponent(window.location.search.substr(1)).split('&').filter(function(item){item != ""}),
        sParameterName,
        exists = false,
        i;
